...
const MomondoSearch = async (depCode: string, arrCode: string, depDate: Date, arrDate: Date): Promise<FlightResultsList | undefined> => {
    ...
    console.log(`https://www.momondo.com/flight-search/${depCode}-${arrCode}/${depDate.toISOString().slice(0, 10)}/${arrDate.toISOString().slice(0, 10)}?sort=bestflight_a`)
...
(async () => {
    // await airportSearch('sfo');
    const list = await MomondoSearch('SJC', 'IAH', new Date('7/13/2023'), new Date('7/16/2023'));
    if (!list) return;
